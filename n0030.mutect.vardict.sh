...
BedFile=primers.NA12878-194-genes-63-indels.10867.bed
bamFile0=/qgen/home/jdicarlo/spe/spe29/62/62.NB500965_0027_0030.oligoClip.bam
bamFile=/qgen/home/xuc/N0030/62.NB500965_0027_0030.oligoClip.RG.bam
runPath=/qgen/home/xuc/N0030/
mt_pct=100
ds_rpb=8.6

# create read groups (RG) 
#echo "Creating read groups..."
#java -Xmx2g -jar /qgen/home/xuc/software/Picard/picard-tools-1.119/AddOrReplaceReadGroups.jar RGLB=lib01 RGPL=illumina RGPU=run RGSM=N0030 I=$bamFile0 O=$bamFile SORT_ORDER=coordinate CREATE_INDEX=TRUE VALIDATION_STRINGENCY=LENIENT > RG.log 2>&1
#samtools index $bamFile

cd $runPath

# ground truth
GroundTruth=$runPath/GIB.NA12878.hc.dil.header.sorted.vcf.gz
...
##python runMutect.new.py   $runPath/vcf $bamFile  $runPath/$BedFile 
#            
## Process output vcfs
#echo "Evaluating peformance..."
#for filter in strict relaxed; do
#   # create results cutoff directory 
#   if [ ! -s result/$caller/$filter ]; then
#      mkdir result/$caller/$filter 
#   fi
#   #for cutoff in 10 50 63 100 200 300 400 500 750 1000 2000 5000; do
#   for cutoff in 1.0 5.0 6.3 10.0 20.0 30.0 40.0 50.0 75.0 100.0 200.0 500.0; do
#      # create results cutoff directory 
#      if [ ! -s result/$caller/$filter/$cutoff ]; then
#         mkdir result/$caller/$filter/$cutoff 
#      fi
#
#      python /qgen/home/xuc/VariantCallingCode/HCandDilute.fast.py  $runPath $BedFile vcf/$caller.$filter.$cutoff.vcf vcf/$caller.$filter.$cutoff.hc.dil.vcf   shared.region.merged.bed HG002-multiall-fullcombine.shared.region.vcf
#      cat muTectHeader.txt vcf/$caller.$filter.$cutoff.hc.dil.vcf > vcf/$caller.$filter.$cutoff.hc.dil.header.vcf
...
#      
#   # get summary results
#   Rscript getSummary.withRTG.R     $runPath  GIB.NA12878.hc.dil.sorted.noheader.vcf  result/$caller/$filter/Table.txt result/$caller/$filter/Table-baseline.txt        903139  $caller:$filter   roc.$caller.$filter.png    summary.$caller.$filter.csv   1>/dev/null 2>>rtg.log	
#
...
#echo "Calling with VarDict at cutoff="$cutoff"..."
   ...
   #${VarDictJava_Path}/build/install/VarDict/bin/VarDict -th 20 -F 0 -q 20 -G $RefGenome -f $cutoff -N N0030 -b $bamFile  -z -c 1 -S 2 -E 3 $runPath/$BedFile | ${VarDictJava_Path}/VarDict/teststrandbias.R | ${VarDictJava_Path}/VarDict/var2vcf_valid.pl -N N0030 -E -f $cutoff > vcf/$caller.$cutoff.vcf 2> error.log

# Process output vcfs
echo "evaluating VCF with RTG..."
   python /qgen/home/xuc/VariantCallingCode/HCandDilute.fast.py  $runPath $BedFile vcf/$caller.$cutoff.vcf vcf/$caller.$cutoff.hc.dil.vcf   shared.region.merged.bed HG002-multiall-fullcombine.shared.region.vcf
   sort -u -k1,4 vcf/$caller.$cutoff.hc.dil.vcf -o vcf/$caller.$cutoff.hc.dil.vcf
   cat VarDictHeader.txt vcf/$caller.$cutoff.hc.dil.vcf > vcf/$caller.$cutoff.hc.dil.header.vcf
...
# get summary results
Rscript getSummary.withRTG.R     $runPath  GIB.NA12878.hc.dil.sorted.noheader.vcf  result/$caller/$filter/Table.txt result/$caller/$filter/Table-baseline.txt        903139  $caller   roc.$caller.png    summary.$caller.csv   1>/dev/null 2>>rtg.log	
