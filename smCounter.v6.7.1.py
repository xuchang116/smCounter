...
# Updates from v6.7: 
# more stringent R1CP/R2CP filter (20 bases to barcode end)
# added lowQ filter (reject candidate variants that have over 40% less than Q20)
# added primerCP filter (optional; reject candidate variants that are within 2 bases from primer)
# Chang Xu. 29Mar2016
...
#-------------------------------------------------------------------------------------
def vc(bamName, chrom, pos, minBQ, minMQ, smt, length, mismatchThr, MTdrop, ds, primerDist):
   ...
   if usedMT == 0:
      ...
      if altPI > min(cutoff) and vtype in ['SNP', 'INDEL']:
         ...
         if vtype=='SNP':
            # distance to primer end of the read
            endBase = primerDist
            ...
            # reject if variant is clustered within 2 bases from primer sequence due to possible primer dimers
...
def main(args):
   ...
   results = [pool.apply_async(vc, args=(args.bamName, x[0], x[1], args.minBQ, args.minMQ, args.StrongMtThr, args.hpLen, args.mismatchThr, args.MTdrop, args.ds, args.primerDist)) for x in locList]
